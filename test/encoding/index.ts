...
export const runEncodingTests = () => {
    it('should encode and decode command `setKey` using secret code', async() => {
        ...
        const { encryptedPacket, packet } = await decodeChunkedPacketV2(chunk(encodedCommand, MAX_PACKET_SIZE - 1), { boxId, secretCode })
    ...
    it('should encode and decode command `unlock` using shared secret key', async() => {
        ...
        const { encryptedPacket, packet } = await decodeChunkedPacketV2(
            chunk(encodedCommand, MAX_PACKET_SIZE - 1),
    ...
    it('should chunk buffer', async() => {
        const buffer = getRandomizedArrayBuffer((MAX_PACKET_SIZE - 1) * 2)

        const chunkedBuffer = await chunkBufferV2(buffer)

        expect(chunkedBuffer.length).to.equal(2)

        expect(chunkedBuffer[0].byteLength).to.equal(MAX_PACKET_SIZE)
        expect(chunkedBuffer[1].byteLength).to.equal(MAX_PACKET_SIZE)
    })

    it('should chunk buffer with 0x0 padding if necessary', async() => {
        const buffer = getRandomizedArrayBuffer(MAX_PACKET_SIZE * 2)

        const chunkedBuffer = await chunkBufferV2(buffer)

        expect(chunkedBuffer.length).to.equal(3)

        for (let offset = 0; offset < 2 * MAX_PACKET_SIZE - 2 * (MAX_PACKET_SIZE - 1); offset++) {
            expect(chunkedBuffer[offset].byteLength).to.equal(MAX_PACKET_SIZE)
        }

        for (let offset = 0; offset < MAX_PACKET_SIZE; offset++) {
            ...
            expect(chunkedBuffer[2].getUint8(offset)).to.equal(buffer[MAX_PACKET_SIZE + (MAX_PACKET_SIZE - 2) + (offset - 1)])
        }
    })

    it('should parse last chunk buffer', async() => {
        const chunk = getRandomizedDataView(MAX_PACKET_SIZE)
        ...
        expect(chunkedPacket.buffer.length).to.equal(MAX_PACKET_SIZE - 1)

        for (let index = 1; index < MAX_PACKET_SIZE; index++) {
    ...
    it('should parse too long chunk buffer', async() => {
        ...
        const chunk = getRandomizedDataView(MAX_PACKET_SIZE + delta)
        chunk.setUint8(PACKET_LAST_INDEX, PACKET_LAST_TRUE)
    ...
    it('should parse too short chunk buffer', async() => {
        ...
        expect(chunkedPacket.isLast).to.be.true
        expect(chunkedPacket.buffer.length).to.equal(MAX_PACKET_SIZE - 1 - delta)

        for (let index = 1; index < MAX_PACKET_SIZE - delta; index++) {
            expect(chunkedPacket.buffer[index - 1]).to.equal(chunk.getUint8(index))
        }
    })
    ...
    it('should decode chunked result', async() => {
        ...
        const result = await decodeChunkedResultV2(chunk(encodedResult, MAX_PACKET_SIZE - 1))
    ...
    it('should decode chunked packet (unencrypted; setKey)', async() => {
        ...
        const { packet } = await decodeChunkedPacketV2(chunk(encodedPacket, MAX_PACKET_SIZE - 1))
